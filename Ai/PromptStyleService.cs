...
namespace OutfitReactions.Ai
{
    ...
    internal sealed class PromptStyleService
    {
        ...
        public static readonly string FallbackActiveFestivalOutfitRule =
            "ACTIVE-FESTIVAL OUTFIT RULE: judge any clear costume, holiday, seasonal, festival, symbolic, creature, or themed look first against {ActiveFestivalName}. When its recognizable motifs or overall concept reasonably fit the active in-game festival, treat it as timely and intentional even if the same motifs could also be associated with a different real-world occasion. Only call it mismatched when it clearly belongs elsewhere and does not reasonably fit the active festival. This compatibility is a private scene fact, not an obligatory topic or opening formula: the NPC may name the festival when natural, but may instead react through a specific visual detail, joke, question, feeling, activity, or character-specific observation without explaining that the look fits. Never say the active festival passed, is upcoming, happens next year, or is not taking place. Do not force a festival comparison for an ambiguous look.";
